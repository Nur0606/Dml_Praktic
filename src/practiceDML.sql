...
update  people set email ='nur@gmail.com' where id=1;
--3 Добавьте новую запись в таблицу
    insert into people(id, first_name, last_name, email, gender, age, language) VALUES (101,'nurmuhammed','rusbaev','nurmuhammed@gmail.com','male',17,'kyrgys');
...
--10 Сгруппировать людей по полу и рассчитать средний возраст для каждого пола
    select gender,avg(age) as srednyi_vozrast from people group by  gender;
...
--12 Посчитать сколько человек, говорящих на казахском(Kazakh)
    select language, count(*) as chislo from people where language='Kazakh' group by language;
--13 Вывести фамилии, почту и айди всех женщин
    select  id,gender ,last_name,email from people where  gender='Female';
--14 Вывести имя и фамилие вместе переименовав как full_name(concat as), пол и год рождение всех мужчин родившихся после 2000 года
    select id,gender,age, concat(first_name,' ',last_name)as full_name from people where age>24;
--15 Посчитать сколько всего имен из таблицы начинаются на букву А
    select  count(*) as kolichestvo from people where first_name like 'A%';
--16 Отсартируйте все почты в алфавитном порядке
    select id, email from people order by email asc;
...
--18 Подсчитать количество людей для каждого языка
    select language, count(*)as count_people from people group by language;
--19 Найти средний возраст для каждого языка, где количество людей больше 2
    select language,avg(age) as srednyi_vozrast from people group by  language having count(*)>2;
--20 Вывести количество записей где почта закончивается на .com
    select count(*) as kolichestvo_zapisey from people where email like '%.com';
--21 Найти самый распространенный язык среди женщин
    select  max(language),count(*) from people where gender='Female';
...
--24 Сгруппируте всех по языку на котором они говорят
    select  language,count(*) as group_by_count from people group by language;
--25 Отобразите записи фамилии которых содержат только 7 символов и отсортируйте их по айди
    select * from people where last_name like '_______' order by id ;
...
select * from people where age>18 and email in (select email  from people where email like'%.com'or email like'%.uk');
--29 Отобразите все записи, кроме родившихся с 1993 по 2000 года
    select  * from people where age not between 24 and 31;
